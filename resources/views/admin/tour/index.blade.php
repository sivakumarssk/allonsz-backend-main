              ...
              <div class="card-body">
                ...
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tr>
                    <td>{{$i}}</td>
                    <td>{{$tour->type}}</td>
                    <td><img src="{{$tour->photo}}" style="height:40px;"></td>
                    <td>{{$tour->name}}</td>
                    <td>{{$tour->place}}</td>
                    <td>{{$tour->area}}</td>
                    <td>{{$tour->price}}</td>
                    <td>{{$tour->desc}}</td>
                    <td>
                      <a href="{{route('tour.show',$tour->id)}}" target="_blank"  class="btn btn-sm btn-warning"><i class="fa fa-eye"></i></a>
                      <button class="btn btn-sm btn-primary" data-toggle="modal" data-target="#addModal" data-id="{{$tour->id}}" data-type="{{$tour->type}}" data-name="{{$tour->name}}" 
                      data-photo="{{$tour->photo}}" data-place="{{$tour->area}}" data-area="{{$tour->area}}" data-price="{{$tour->price}}" data-desc="{{$tour->desc}}"><i class="fa fa-edit"></i></button>
                      @if($tour->deleted_at)
                      <button class="btn btn-sm btn-success" data-toggle="modal" data-target="#deleteModal" data-id="{{$tour->id}}" data-action="restore"><i class="fa fa-undo"></i></button>
                      @else
                      <button class="btn btn-sm btn-danger" data-toggle="modal" data-target="#deleteModal" data-id="{{$tour->id}}" data-action="delete"><i class="fa fa-trash"></i></button>
                      @endif
                    </td>
...
<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="{{route('tour.store')}}" method="post" class="add-form" enctype="multipart/form-data">
        @csrf
